...
const Breadcrumb = () => {
  const pathname = usePathname(); // `usePathname` replaces `router.asPath`
  const paths = pathname.split("/").filter((item) => item !== ""); // Filter out empty strings from split
          ...
          <li key={index}>
            <div className="flex items-center">
              ...
              {index === paths.length - 1 ? (
                <span className="ml-1 md:ml-2 text-sm font-medium capitalize text-regularAccent">
                  {item}
                </span>
              ) : (
