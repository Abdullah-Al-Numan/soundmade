...
const LinkEdit = ({ socialLink, userId }: { socialLink: ArtistValidation; userId: string }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(graphQLError || "Failed to update links, please try again.");
    } finally {
  ...
  const renderInput = (label: string, name: keyof typeof links) => (
    <div className="text-start" key={name}>
      <Label>{label}</Label>
      <Input name={name} placeholder={`Enter ${label} Link`} value={links[name]} onChange={handleChange} />
    </div>
  );

  return (
    <section className="flex justify-center mt-5">
      <form onSubmit={handleSubmit} className="w-full max-w-md space-y-4 border p-6 rounded-lg shadow-lg">
        <p className="text-2xl text-center font-bold text-gray-900">Update Social Url</p>
